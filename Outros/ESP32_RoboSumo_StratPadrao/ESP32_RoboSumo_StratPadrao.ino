...
void lerSensores() {
    ...
    // Sensores de linha: LOW = Borda Branca, HIGH = Preto
...
void controleAutomatico() {
    // Detecta borda (LOW = branco, HIGH = preto)
    ...
    switch (estadoAtualAuto) {
        ...
        case RE:
            moverTras();
            if (millis() - tempoAcaoAuto >= 500) { // Dá ré por 0.5 segundos
                pararMotores();
                tempoAcaoAuto = millis();
                estadoAtualAuto = GIRAR_ALEATORIO;
                Serial.println("Re concluida, girando aleatoriamente.");
            }
            break;
        ...
        case GIRAR_ALEATORIO:
            ...
            if (millis() - tempoAcaoAuto >= 700) { // Gira por 0.7 segundos
                pararMotores();
                estadoAtualAuto = FRENTE;
                Serial.println("Giro concluido, voltando para FRENTE.");
            }
            break;
    }
}
