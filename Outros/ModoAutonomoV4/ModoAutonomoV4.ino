...
void setup() {
  ...
  // NOVO CÓDIGO: Inicia o Modo Padrão após um atraso inicial
  Serial.println("Aguardando 5 segundos para iniciar o Modo Padrao automaticamente...");
  
  // Define o modo de operação e o estado inicial para esperar
  modoOperacaoAutonomoAtual = MODO_AUTONOMO_PADRAO;
  mudarEstado(AUTONOMO_ESPERANDO_INICIO, "Espera 5s (Auto)"); 
  // O delay de 5 segundos não é feito aqui diretamente para não bloquear o setup.
  // A máquina de estados no loop cuidará do TEMPO_ESPERA_INICIAL_AUTONOMO_MS.
}
